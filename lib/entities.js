...
var player = function(world, id, username) {
  ...
  offsetX = asteroidCenter.x + 1+ leastPopAst.radius/box2DScale;
  offsetY = asteroidCenter.y + 1+ leastPopAst.radius/box2DScale;
  ...
  self.body.SetAngle(Math.atan((bodyDef.position.y - asteroidCenter.y)/(bodyDef.position.x-asteroidCenter.x))+Math.PI);
