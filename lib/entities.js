...
var player = function(world, id, username, x, y) {
  var self = this;
  self.type = 'player';
  ...
  // TODO: ADD ASTEROID RANDOMIZATION LOGIC
  for (var i=0; i < world.asteroids.length; i++) {
    distToAsteroid = new b2Vec2(0, 0);
    distToAsteroid.Add(world.asteroids[i].body.GetWorldCenter());
    distToAsteroid.Subtract(self.body.GetWorldCenter());
  }

  // TODO remove me
  var kick = new b2Vec2(35+Math.random(), -5);
...
var missile = function(world, player, x, y, angle, power) {
  var self = this;
  ...
  self.angle = self.body.GetAngle(angle*Math.PI/180);
  self.toJSON = function() {
    var safeJSON = _.clone(self);
