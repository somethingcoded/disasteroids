...
var world = function() {
  ...
  self.addAsteroids = function() {
    ...
    var minRadius = 60;
    var maxRadius = 120;
    var sectionPaddingX = 200;
    var sectionPaddingY = 220;
    var row = 0;
    var col = 0;
    var maxRow = Math.round((maxY - edgeOffset*2)/(edgeOffset+sectionPaddingY));
    var maxCol = Math.round((maxX - edgeOffset*2)/(edgeOffset+sectionPaddingX));
    console.log('rows: ' + maxRow + ' cols: ' + maxCol);

    for (var i=0; i < maxCol; i++) {
      for (var j=0; j < maxRow; j++) {
        var xMin = (edgeOffset*(i+1)+(sectionPaddingX*i));
        var xMax = (edgeOffset*(i+2)+(sectionPaddingX*i));
        var yMin = (edgeOffset*(j+1)+(sectionPaddingY*j));
        var yMax = (edgeOffset*(j+2)+(sectionPaddingY*j));
        var xPos = getRandomInt(xMin, xMax);
        var yPos = getRandomInt(yMin, yMax);
        var rad = getRandomInt(minRadius, maxRadius);
